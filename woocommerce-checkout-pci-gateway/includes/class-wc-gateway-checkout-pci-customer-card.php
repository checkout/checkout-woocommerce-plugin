 ...
 *
 * Class WC_Checkout_Pci_Customer_Card
 *
 * @version 20160317
 */
class WC_Checkout_Pci_Customer_Card
{
    ...
    public static function getCustomerCardListHtml($customerId) {
        ...
        $actionUrl  = get_site_url() . '/wp-content/plugins/woocommerce-checkout-pci-gateway/controllers/customer/card/delete.php';
        $result     = '';
        ...
        $result .= "<h2>" . __("My Saved Cards (Checkout.com)", 'woocommerce-checkout-pci') . "</h2>";
        $result .= "<table class='shop_table shop_table_responsive my_account_checkout_cards my_account_orders'>";
        /* START: Head */
        $result .= "<thead>";
        $result .= "<tr>";
        $result .= "<th class='card-number'>" . __("Card #", 'woocommerce-checkout-pci') . "</th>";
        $result .= "<th class='card-type'>" . __("Type", 'woocommerce-checkout-pci') . "</th>";
