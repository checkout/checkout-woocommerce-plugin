		...
		?>
			<div class="ideal-bank-info" id="ideal-bank-info">
				<div class="ideal-heading">
					<label><?php esc_html_e( 'Your Bank', 'uprise-payment-woocommerce' ); ?></label>
				</div>
				<label for="issuer-id">
		...
		<div class="klarna-details">
			<div class="klarna_widgets">
				<?php if ( ! empty( $payment_method_categories ) ) { ?>
					...
					echo  __( 'Klarna is not offering any payment options for this purchase. Please choose another payment method.', 'uprise-payment-woocommerce' );
				}
				?>
			</div>
		</div>
	...
	public static function get_boleto_details() {
		...
		<div data-role="content" class="boleto-content">
			<div class="input-group">
				...
				<input type="text" id="name" name="name" placeholder="<?php echo ( __( 'Nome', 'uprise-payment-woocommerce' ) ); ?>" class="input-control" required style="width: 100%;">
			</div>
			<div class="input-group">
				<label class="icon" for="cpf">
					<span class="ckojs ckojs-card"></label>
				<input type="text" id="cpf" name="cpf" placeholder="<?php echo ( __( 'Cadastro de Pessoas Físicas', 'uprise-payment-woocommerce' ) ); ?>" class="input-control" required style="width: 100%;">
			</div>
		</div>
		<?php
	}
		...
		<div class="sepa-content">
			<div class="input-group">
				<label class="icon" for="sepa-iban">
					<span class="ckojs ckojs-card"></label>
				<input type="text" id="sepa-iban" name="sepa-iban" placeholder="<?php echo ( __( 'IBAN', 'uprise-payment-woocommerce' ) ); ?>" class="input-control" required style="width: 100%;">
			</div>
			...
			$alert = esc_html__( 'Please fill in the required fields.', 'uprise-payment-woocommerce' );
			?>
		</div>
