...
use CheckoutComWC\Vendor\GuzzleHttp\Cookie\CookieJar;
...
use CheckoutComWC\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
use CheckoutComWC\Vendor\GuzzleHttp\Handler\CurlShareHandleState;
use CheckoutComWC\Vendor\GuzzleHttp\Handler\CurlVersion;
 ...
 * @final
 */
class Client implements ClientInterface, \CheckoutComWC\Vendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    {
        checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
        if (\count($args) < 1) {
        ...
        $method = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
    ...
    {
        $normalizedMethod = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
        ...
        if ($droppedHeaderNames !== [] && isset($options['_conditional'])) {
            $options['_conditional'] = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove($droppedHeaderNames, $options['_conditional']);
        }
        $body = $options['body'] ?? null;
        $version = self::normalizeProtocolVersion($options['version'] ?? '1.1');
        // Merge the URI into the base URI.
        $uri = $this->buildUri(\CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = true;
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\UriResolver::resolve(\CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function configureDefaults(array $config): void
    {
        ...
        if (!isset($this->config['headers'])) {
            ...
            foreach (\array_keys($this->config['headers']) as $name) {
                if (\CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower((string) $name) === 'user-agent') {
                    $hasUserAgent = true;
                    break;
                }
            }
    ...
    private static function warnAboutRequestLevelHandler(array $options): void
    {
        ...
        checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $request = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (isset($options['decode_content']) && \is_string($options['decode_content'])) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            $value = $options['auth'];
            $type = isset($value[2]) ? \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::asciiToLower($value[2]) : 'basic';
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. NTLM is also deprecated by curl/libcurl and may be unavailable in current or future libcurl builds. Avoid NTLM; if you must use it temporarily, configure cURL HTTP authentication options directly with a libcurl build that still supports NTLM.');
        ...
        $request = \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \CheckoutComWC\Vendor\GuzzleHttp\Psr7\MultipartStream) {
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        if (\is_resource($body) || $body === null || \is_string($body) || $body instanceof \Iterator) {
            return \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::streamFor($body);
        }
        if (\is_scalar($body)) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
            return \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::streamFor(self::stringifyScalar($body));
        }
        if (\is_object($body) && \method_exists($body, '__toString')) {
            return \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::streamFor((string) $body);
        }
        if (\is_callable($body)) {
            return \CheckoutComWC\Vendor\GuzzleHttp\Psr7\Utils::streamFor($body);
        }
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option): array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            checkoutcomwc_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
