...
include_once 'class-wc-uprisepay-utility.php';
include 'class-wc-uprisepay-apm-method.php';
...
class WC_Checkoutcom_Api_Request {
	...
	public static function create_payment( WC_Order $order, $arg, $subscription = null ) {
		...
		$is_sepa_renewal       = ( 'wc_uprise_payment_alternative_payments_sepa' === $order_payment_method ) && ( ! is_null( $subscription ) );
		$is_google_pay_renewal = ( 'wc_uprise_payment_google_pay' === $order_payment_method ) && ( ! is_null( $subscription ) );
		$is_apple_pay_renewal  = ( 'wc_uprise_payment_apple_pay' === $order_payment_method ) && ( ! is_null( $subscription ) );
		$is_paypal_renewal     = ( 'wc_uprise_payment_paypal' === $order_payment_method ) && ( ! is_null( $subscription ) );
		...
		try {
			...
			if ( WC_Checkoutcom_Utility::is_successful( $response ) ) {
				...
				if ( WC_Checkoutcom_Utility::is_pending( $response ) ) {
					...
					if ( WC_Checkoutcom_Utility::get_redirect_url( $response ) ) {
						...
						$error_message = __( 'An error has occurred while processing your payment. Redirection link not found', 'uprise-payment-woocommerce' );
				...
				if ( 'Declined' === $response['status'] ) {
					update_post_meta( $order->get_id(), '_upy_payment_id', $response['id'] );
				}

				$error_message = __( 'An error has occurred while processing your payment. Please check your card details and try again. ', 'uprise-payment-woocommerce' );
				...
				if ( $gateway_debug ) {
					...
					if ( ! preg_match( '/^(?:40)\d+$/', $response['response_code'] ) ) {
						...
						$error_message .= sprintf( __( 'Status : %1$s, Response summary : %2$s', 'uprise-payment-woocommerce' ), $response['status'], $response['response_summary'] );
					}
				}
			...
			$error_message = __( 'An error has occurred while processing your payment. ', 'uprise-payment-woocommerce' );
	...
	private static function get_request_param( WC_Order $order, $arg, $subscription = null ) {
		...
		$auto_capture       = '1' === WC_Admin_Settings::get_option( 'upycom_card_autocap', '1' );
		$amount             = $order->get_total();
		$amount_cents       = WC_Checkoutcom_Utility::value_to_decimal( $amount, $order->get_currency() );
		$three_d            = '1' === WC_Admin_Settings::get_option( 'upycom_card_threed', '0' ) && null === $subscription;
		$attempt_no_three_d = '1' === WC_Admin_Settings::get_option( 'upycom_card_notheed', '0' );
		$dynamic_descriptor = '1' === WC_Admin_Settings::get_option( 'upycom_card_desctiptor', '0' );
		$mada_enable        = '1' === WC_Admin_Settings::get_option( 'upycom_card_mada', '0' );
		$save_card          = WC_Admin_Settings::get_option( 'upycom_card_saved' );
		$google_settings    = get_option( 'woocommerce_wc_uprise_payment_google_pay_settings' );
		$is_google_threeds  = 1 === absint( $google_settings['upycom_google_threed'] ) && null === $subscription;
		$is_paypal_renewal  = ( 'wc_uprise_payment_paypal' === $order->get_payment_method() ) && ( ! is_null( $subscription ) );

		$is_save_card   = false;
		$payment_option = 'FramesJs';
		$apms_settings  = get_option( 'woocommerce_wc_uprise_payment_alternative_payments_settings' );
		$apms_selected  = ! empty( $apms_settings['upycom_apms_selector'] ) ? $apms_settings['upycom_apms_selector'] : [];
		...
		if ( 'wc_uprise_payment_cards' === $post_data['payment_method'] ) {
			if ( $post_data['wc-wc_uprise_payment_cards-payment-token'] ) {
				if ( 'new' === $post_data['wc-wc_uprise_payment_cards-payment-token'] ) {
					// New card used.
					...
					if ( WC_Admin_Settings::get_option( 'upycom_card_require_cvv' ) ) {
						$method->cvv = $post_data['wc_uprise_payment_cards-card-cvv'];
					}
				}
			} else {
				$method        = new RequestTokenSource();
		...
		} elseif ( 'wc_uprise_payment_google_pay' === $post_data['payment_method'] ) {
			$payment_option = 'Google Pay';

			$method        = new RequestTokenSource();
			$method->token = trim( $arg['token'] );

		} elseif ( 'wc_uprise_payment_apple_pay' === $post_data['payment_method'] ) {
			$payment_option = 'Apple Pay';

			$method        = new RequestTokenSource();
			$method->token = trim( $arg );

		} elseif ( 'wc_uprise_payment_paypal' === $post_data['payment_method'] ) {
		...
		if ( ! is_null( $subscription ) ) {
			...
			$payment->previous_payment_id = get_post_meta( $arg['parent_order_id'], '_upy_payment_id', true ) ?? null;
		...
		if ( 'wc_uprise_payment_cards' === $post_data['payment_method']
			...
			&& $save_card
			&& (
				isset( $post_data['wc-wc_uprise_payment_cards-new-payment-method'] )
				&& sanitize_text_field( $post_data['wc-wc_uprise_payment_cards-new-payment-method'] )
			)
		) {
		...
		if ( $dynamic_descriptor ) {
			$descriptor_name = WC_Admin_Settings::get_option( 'upycom_card_desctiptor_name' );
			$descriptor_city = WC_Admin_Settings::get_option( 'upycom_card_desctiptor_city' );
		...
		if ( 'wc_uprise_payment_google_pay' === $post_data['payment_method'] && ( $is_google_threeds && 'pan_only' === $arg['token_format'] ) ) {
		...
		if ( upy_is_nas_account() ) {
			$redirection_url = home_url( '/uprisepay-callback' );
		}
	...
	public static function verify_session( $session_id ) {
		...
		try {
			...
			if ( WC_Checkoutcom_Utility::is_successful( $response ) ) {
				...
				// Set payment id post meta if the payment id declined.
				if ( 'Declined' === $response['status'] ) {
					update_post_meta( $response['metadata']['order_id'], '_upy_payment_id', $response['id'] );
				}
				...
				if ( $gateway_debug ) {
					if ( ! empty( $response['actions'] ) ) {
						...
						/* translators: 1: Response status, 2: Summary. */
						$error_message .= sprintf( __( 'Status : %1$s, Response summary : %2$s', 'uprise-payment-woocommerce' ), $response['status'], $action['response_summary'] );
					}
				}
	...
	public static function generate_google_token() {
		...
		try {
			...
			$error_message = __( 'An error has occurred while processing your Google pay payment.', 'uprise-payment-woocommerce' );
			WC_Checkoutcom_Utility::logger( $error_message, $ex );
		}
	}
	...
	public static function capture_payment() {
		...
		$upy_payment_id = get_post_meta( $order_id, '_upy_payment_id', true );

		// Check if upy_payment_id is empty.
		if ( empty( $upy_payment_id ) ) {
			$error_message = esc_html__( 'An error has occurred. No Cko Payment Id', 'uprise-payment-woocommerce' );
		...
		try {
			...
			$details = $checkout->get_builder()->getPaymentsClient()->getPaymentDetails( $upy_payment_id );

			if ( 'Voided' === $details['status'] || 'Captured' === $details['status'] ) {
				$error_message = sprintf(
					/* translators: 1: Order ID. */
					esc_html__( 'Payment has already been voided or captured on Uprise Payment portal for order Id : %s', 'uprise-payment-woocommerce' ),
					$order_id
				);
			...
			$response = $checkout->get_builder()->getPaymentsClient()->capturePayment( $upy_payment_id, $capture_request );

			if ( ! WC_Checkoutcom_Utility::is_successful( $response ) ) {
				$error_message = sprintf(
					/* translators: 1: Order ID. */
					esc_html__( 'An error has occurred while processing your capture payment on Uprise Payment portal. Order Id : %s', 'uprise-payment-woocommerce' ),
					$order_id
				);
			...
			$error_message = esc_html__( 'An error has occurred while processing your capture request.', 'uprise-payment-woocommerce' );
	...
	public static function void_payment() {
		...
		try {
			...
			// Process void payment on Uprise.
			$response = $checkout->get_builder()->getPaymentsClient()->voidPayment( $upy_payment_id, $void_request );

			if ( ! WC_Checkoutcom_Utility::is_successful( $response ) ) {
				$error_message = sprintf(
					/* translators: 1: Order ID. */
					esc_html__( 'An error has occurred while processing your void payment on Uprise Payment portal. Order Id : %s', 'uprise-payment-woocommerce' ),
					$order_id
				);
			...
			$error_message = esc_html__( 'An error has occurred while processing your void request.', 'uprise-payment-woocommerce' );
	...
	public static function refund_payment( $order_id, $order ) {
		...
		// Check if upy_payment_id is empty.
		if ( empty( $upy_payment_id ) ) {
			$error_message = __( 'An error has occurred. No Cko Payment Id', 'uprise-payment-woocommerce' );
		...
		try {
			...
			$response = $checkout->get_builder()->getPaymentsClient()->refundPayment( $upy_payment_id, $refund_request );

			if ( ! WC_Checkoutcom_Utility::is_successful( $response ) ) {
				/* translators: 1: Order ID. */
				$error_message = sprintf( esc_html__( 'An error has occurred while processing your refund payment on Uprise Payment portal. Order Id : %s', 'uprise-payment-woocommerce' ), $order_id );
			...
			$error_message = esc_html__( 'An error has occurred while processing your refund. ', 'uprise-payment-woocommerce' );
	...
	public static function get_ideal_bank() {
		...
		try {
			...
			$error_message = __( 'An error has occured while retrieving ideal bank details.', 'uprise-payment-woocommerce' );
	...
	public static function create_apm_payment( WC_Order $order, $payment_method ) {
		...
		try {
			...
			if ( WC_Checkoutcom_Utility::is_successful( $response ) ) {
				...
				if ( WC_Checkoutcom_Utility::is_pending( $response ) || 'Authorized' === $response['status'] ) {
					...
					if ( WC_Checkoutcom_Utility::get_redirect_url( $response ) ) {
						...
						$error_message = esc_html__( 'An error has occurred while processing your payment. Redirection link not found', 'uprise-payment-woocommerce' );
				...
				$error_message = esc_html__( 'An error has occurred while processing your payment. Please check your card details and try again.', 'uprise-payment-woocommerce' );
			...
			$error_message = esc_html__( 'An error has occurred while creating apm payments.', 'uprise-payment-woocommerce' );
	...
	public static function klarna_session() {
		...
		try {
			...
			$error_message = esc_html__( 'An error has occurred while creating klarna session.', 'uprise-payment-woocommerce' );
	...
	public static function generate_apple_token() {
		...
		try {
			...
			$error_message = esc_html__( 'An error has occurred while processing your payment.', 'uprise-payment-woocommerce' );
			WC_Checkoutcom_Utility::logger( $error_message, $ex );
			die( 'here' );
		}
	}
	...
	public static function mandate_cancel_request( $url, $subscription_id ) {
		...
		$wp_request_headers = [
			'Authorization' => $core_settings['upycom_sk'],
		];
