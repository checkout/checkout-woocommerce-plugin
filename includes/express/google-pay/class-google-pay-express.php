...
class CKO_Google_Pay_Express {
	...
	public function display_payment_request_button_html() {
		...
		<div id="cko-google-pay-button-wrapper" style="margin-top: 1em;clear:both;"></div>
		<?php
	}
	...
	public function display_cart_payment_request_button_html() {
		...
		// Check if unified container has been rendered by PayPal
		$unified_container_rendered = WC_Checkoutcom_Utility::express_checkout_container_rendered();
		
		$should_render_container = ! $unified_container_rendered && ( $paypal_express_enabled && $paypal_available && $paypal_show_on_cart );
		...
		if ( $should_render_container ) {
			...
			// Unified container already rendered by PayPal, add Google Pay button via JavaScript
			?>
			<script>
			(function() {
				...
				if (buttonsContainer && !document.getElementById('cko-google-pay-button-wrapper-cart')) {
					...
					buttonsContainer.appendChild(googleWrapper);
				}
			})();
			</script>
			<?php
