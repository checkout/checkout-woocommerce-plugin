...
/**
 * Class WC_Gateway_Uprise_Payment_Alternative_Payments_Sepa
 *
 * @class   WC_Gateway_Uprise_Payment_Alternative_Payments_Sepa
 * @extends WC_Gateway_Uprise_Payment_Alternative_Payments
 */
class WC_Gateway_Uprise_Payment_Alternative_Payments_Sepa extends WC_Gateway_Uprise_Payment_Alternative_Payments {
	...
	public function __construct() {
		$this->id                 = 'wc_uprise_payment_alternative_payments_sepa';
		...
		$this->title              = __( 'SEPA Direct Debit', 'uprise-payment-woocommerce' );
	...
	public function payment_fields() {
		...
		if ( ! in_array( self::PAYMENT_METHOD, $apm_available, true ) ) {
			?>
				<script>
					jQuery('.payment_method_wc_uprise_payment_alternative_payments_sepa').hide();
				</script>
			<?php
		} else {
			...
			?>
				...
				jQuery('#place_order').click(function(e) {
					...
					if (jQuery('#payment_method_wc_uprise_payment_alternative_payments_sepa').is(':checked')) {
	...
	public function process_payment( $order_id ) {
		...
		if ( ! empty( $result['source'] ) && self::PAYMENT_METHOD === $result['source']['type'] ) {
			...
			update_post_meta( $order_id, 'upy_sepa_mandate_reference', $mandate );
			update_post_meta( $order_id, 'upy_payment_authorized', true );

			$message = sprintf(
				/* translators: 1: Result ID, 2: Mandate reference. */
				esc_html__( 'Uprise - Sepa payment Action ID : %1$s - Sepa mandate reference : %2$s', 'uprise-payment-woocommerce' ),
				$result['id'],
				$mandate
			);
