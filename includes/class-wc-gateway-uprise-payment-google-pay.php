...
/**
 * Class WC_Gateway_Uprise_Payment_Google_Pay for Google Pay payment method.
 */
class WC_Gateway_Uprise_Payment_Google_Pay extends WC_Payment_Gateway {

	/**
	 * WC_Gateway_Uprise_Payment_Google_Pay constructor.
	 */
	public function __construct() {
		$this->id                 = 'wc_uprise_payment_google_pay';
		...
		$this->title              = __( 'Google Pay', 'uprise-payment-woocommerce' );
	...
	public function payment_scripts() {
		...
		if ( $google_pay_enabled ) {
			...
			$environment   = 'sandbox' === $core_settings['upycom_environment'];
			$currency_code = get_woocommerce_currency();
			$total_price   = WC()->cart->total;

			$vars = [
				'environment'   => $environment ? 'TEST' : 'PRODUCTION',
				'public_key'    => $core_settings['upycom_pk'],
				'merchant_id'   => $this->get_option( 'upycom_google_merchant_id' ),
				'currency_code' => $currency_code,
				'total_price'   => $total_price,
				'button_type'   => $this->get_option( 'upycom_google_style' ),
			];

			wp_localize_script( 'cko-google-pay-integration-script', 'upy_google_pay_vars', $vars );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		// Get cko auth status configured in admin.
		$status = WC_Admin_Settings::get_option( 'upycom_order_refunded', 'refunded' );

		/* translators: %s: Action ID. */
		$message = sprintf( esc_html__( 'Uprise Payment refunded - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] );

		if ( isset( $_SESSION['cko-refund-is-less'] ) ) {
			if ( $_SESSION['cko-refund-is-less'] ) {
				/* translators: %s: Action ID. */
				$order->add_order_note( sprintf( esc_html__( 'Uprise Payment Partially refunded - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] ) );
