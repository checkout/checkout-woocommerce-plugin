...
use UprisePayment\UprisePaymentApiException;

/**
 * Class WC_Uprise_Payment_Webhook handles webhook events from Checkout.com.
 */
class WC_Uprise_Payment_Webhook {
	...
	public static function authorize_payment( $data ) {
		...
		$already_captured = get_post_meta( $order_id, 'upy_payment_captured', true );

		if ( $already_captured ) {
			return true;
		}

		$already_authorized = get_post_meta( $order_id, 'upy_payment_authorized', true );
		$auth_status        = WC_Admin_Settings::get_option( 'upycom_order_authorised', 'on-hold' );
		...
		update_post_meta( $order_id, '_upy_payment_id', $webhook_data->id );
	...
	public static function card_verified( $data ) {
		...
		$order->add_order_note( __( 'Uprise Card verified webhook received', 'uprise-payment-woocommerce' ) );
		// Set action id as woo transaction id.
		update_post_meta( $order_id, '_transaction_id', $action_id );
	...
	public static function capture_payment( $data ) {
		...
		*/
		...
		$order->add_order_note( __( 'Uprise Payment Capture webhook received', 'uprise-payment-woocommerce' ) );
		...
		$order_message = sprintf( esc_html__( 'Uprise Payment Captured - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );

		// Check if webhook amount is less than order amount.
		if ( $amount < $order_amount_cents ) {
			/* translators: %s: Action ID. */
			$order_message = sprintf( esc_html__( 'Uprise Payment partially captured - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );
		}
	...
	public static function void_payment( $data ) {
		...
		// check if payment is already captured.
		$already_voided = get_post_meta( $order_id, 'upy_payment_voided', true );
		...
		$order->add_order_note( esc_html__( 'Uprise Payment Void webhook received', 'uprise-payment-woocommerce' ) );
		...
		// Set action id as woo transaction id.
		update_post_meta( $order_id, '_transaction_id', $action_id );
		update_post_meta( $order_id, 'upy_payment_voided', true );

		// Get cko capture status configured in admin.
		$status = WC_Admin_Settings::get_option( 'upycom_order_void', 'cancelled' );

		/* translators: %s: Action ID. */
		$order_message = sprintf( esc_html__( 'Uprise Payment Voided - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );
	...
	public static function refund_payment( $data ) {
		...
		$already_refunded = get_post_meta( $order_id, 'upy_payment_refunded', true );
		...
		$order->add_order_note( esc_html__( 'Uprise Payment Refund webhook received', 'uprise-payment-woocommerce' ) );
		...
		/* translators: %s: Action ID. */
		$order_message = sprintf( esc_html__( 'Uprise Payment Refunded - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );

		// Check if webhook amount is less than order amount - partial refund.
		if ( $amount < $order_amount_cents ) {
			/* translators: %s: Action ID. */
			$order_message = sprintf( esc_html__( 'Uprise Payment partially refunded - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );
			...
			// Full refund.
			/* translators: %s: Action ID. */
			$order_message = sprintf( esc_html__( 'Uprise Payment fully refunded - Action ID : %s', 'uprise-payment-woocommerce' ), $action_id );
	...
	public static function cancel_payment( $data ) {
		...
		} catch ( UprisePaymentApiException $ex ) {
