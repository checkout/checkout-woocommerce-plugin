...
class WC_Gateway_Checkout_Com_Alternative_Payments_Poli extends WC_Gateway_Checkout_Com_Alternative_Payments {
    ...
    {   
        ...
        ?>
            <script>
                jQuery('.payment_box.payment_method_wc_checkout_com_alternative_payments_poli').attr("style", "visibility: hidden;");
            </script>
            <input type="hidden" id="cko-apm" name="cko-apm" value="poli">
        <?php

        if (! in_array("poli", $apm_available) ) {
            ?>
                <script>
