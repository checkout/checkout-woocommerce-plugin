...
class WC_Gateway_Checkout_Com_Alternative_Payments_Bancontact extends WC_Gateway_Checkout_Com_Alternative_Payments {
    ...
    {   
        ...
        ?>
            <script>
                jQuery('.payment_box.payment_method_wc_checkout_com_alternative_payments_bancontact').attr("style",
                "visibility: hidden;");
            </script>
            <input type="hidden" id="cko-apm" name="cko-apm" value="bancontact">
        <?php

        if (! in_array("bancontact", $apm_available) ) {
            ?>
                <script>
