...
include_once 'class-wc-uprisepay-workflows.php';
...
class WC_Checkoutcom_Webhook {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_wc_uprisepay_register_webhook', [ $this, 'ajax_register_webhook' ] );
		add_action( 'wp_ajax_wc_uprisepay_check_webhook', [ $this, 'ajax_check_webhook' ] );

		$this->account_type = upy_is_nas_account() ? 'NAS' : 'ABC';
	...
	public function ajax_register_webhook() {
		...
		check_ajax_referer( 'uprisepay_register_webhook', 'security' );
	...
	public function create( $url ) {
		...
		try {
			...
			$gateway_debug = WC_Admin_Settings::get_option( 'upy_gateway_responses' ) === 'yes';

			$error_message = esc_html__( 'An error has occurred while processing webhook request.', 'uprise-payment-woocommerce' );
	...
	public static function generate_current_webhook_url(): string {
		return add_query_arg( 'wc-api', 'wc_uprisepay_webhook', home_url( '/' ) );
	}
	...
	public function ajax_check_webhook() {
		...
		check_ajax_referer( 'uprisepay_check_webhook', 'security' );

		if ( 'ABC' === $this->account_type ) {
			$webhook_is_ready = $this->is_registered();

			$message = esc_html__( 'Webhook is configured at this URL:', 'uprise-payment-woocommerce' );
		} else {
			// NAS account type.
			// @todo: Use SDK to get webhooks or workflows.
			$webhook_is_ready = WC_Checkoutcom_Workflows::get_instance()->is_registered();

			$message = esc_html__( 'Webhook is configured with this name:', 'uprise-payment-woocommerce' );
		}

		if ( $webhook_is_ready ) {

			$message = $message ? $message : esc_html__( 'Webhook is configured at this URL:', 'uprise-payment-woocommerce' );
			$message = sprintf( '%s <code>%s</code>', $message, $webhook_is_ready );
		} else {

			$message = esc_html__( 'Webhook is not configured with the current site or there is some issue with connection, Please check logs or try again.', 'uprise-payment-woocommerce' );
		}
