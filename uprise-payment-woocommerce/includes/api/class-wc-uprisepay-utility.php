...
class WC_Checkoutcom_Utility {
	 ...
	 * @param string $upy_signature Header signature.
	 *
	 * @return bool
	 */
	public static function verify_signature( $event, $key, $upy_signature ) {
		return hash_hmac( 'sha256', $event, $key ) === $upy_signature;
	}
	...
	public static function get_delayed_capture_timestamp() {
		...
		$delay                 = preg_replace( '/\s/', '', WC_Admin_Settings::get_option( 'upycom_card_cap_delay' ) );
	...
	public static function logger( $error_message, $exception ) {
		...
		$file_logging = 'yes' === WC_Admin_Settings::get_option( 'upy_file_logging', 'no' );
	...
	public static function get_alternative_payment_methods() {
		...
		$apm_setting   = get_option( 'woocommerce_wc_uprise_payment_alternative_payments_settings' );
		$apm           = ! empty( $apm_setting['upycom_apms_selector'] ) ? $apm_setting['upycom_apms_selector'] : [];
