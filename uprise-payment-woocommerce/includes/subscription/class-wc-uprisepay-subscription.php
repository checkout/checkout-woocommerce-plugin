...
include_once WC_CHECKOUTCOM_PLUGIN_PATH . '/includes/api/class-wc-uprisepay-api-request.php';
...
class WC_Checkoutcom_Subscription {
	...
	public static function renewal_payment( $renewal_total, $renewal_order ) {
		...
		foreach ( $subscriptions_arr as $subscriptions_obj ) {
			$args['source_id']        = get_post_meta( $subscriptions_obj->get_id(), '_upy_source_id', true );
			$args['parent_order_id']  = $subscriptions_obj->get_parent_id();
			$args['preferred_scheme'] = get_post_meta( $subscriptions_obj->get_id(), '_upy_preferred_scheme', true );
		}
	...
	public static function update_order_status( $payment_result, $renewal_order, $order_id ) {
		...
		update_post_meta( $order_id, '_upy_payment_id', $payment_result['id'] );

		// Set Authorize meta key to true if renewal order.
		if ( in_array(
			get_post_meta( $order_id, '_payment_method', true ),
			[
				'wc_uprise_payment_google_pay',
				'wc_uprise_payment_apple_pay',
				'wc_uprise_payment_alternative_payments_sepa',
			],
			true
		) ) {
		...
		$message = sprintf(
			...
			__( 'Uprise Payment Authorised - Action ID : %s ', 'uprise-payment-woocommerce' ),
		...
		if ( $payment_result['risk']['flagged'] ) {
			...
			$message = sprintf(
				/* translators: %s: Payment result ID. */
				__( 'Uprise Payment Flagged - Action ID : %s ', 'uprise-payment-woocommerce' ),
				$payment_result['action_id']
			);
		}
	...
	public static function save_source_id( $order_id, $order, $source_id ) {
		...
		if ( $order instanceof WC_Subscription ) {
			update_post_meta( $order->get_id(), '_upy_source_id', $source_id );
		}
		...
		if ( function_exists( 'wcs_order_contains_subscription' ) ) {
			if ( wcs_order_contains_subscription( $order_id ) ) {
				...
				foreach ( $subscriptions as $subscription_obj ) {
					update_post_meta( $subscription_obj->get_id(), '_upy_source_id', $source_id );
				}
			}
		}
	...
	public static function subscription_cancelled( $subscription ) {
		...
		if ( 'wc_uprise_payment_alternative_payments_sepa' === $subscription->get_payment_method() ) {

			$mandate_cancel = get_post_meta( $subscription->get_id(), '_upy_mandate_cancel', true );
	...
	public static function save_mandate_cancel( $order_id, $order, $url ) {
		...
		if ( $order instanceof WC_Subscription ) {
			update_post_meta( $order->get_id(), '_upy_mandate_cancel', $url );
		}
		...
		if ( function_exists( 'wcs_order_contains_subscription' ) ) {
			if ( wcs_order_contains_subscription( $order_id ) ) {
				...
				foreach ( $subscriptions as $subscription_obj ) {
					update_post_meta( $subscription_obj->get_id(), '_upy_mandate_cancel', $url );
				}
			}
		}
	}
