...
jQuery("#place_order").click(function (e) {
  ...
  if (
    jQuery("#payment_method_wc_uprise_payment_alternative_payments_klarna").is(
      ":checked"
    )
  ) {
    // Klarna
    if (
      jQuery(".klarna_widgets").length > 0 &&
      jQuery(".klarna_widgets").find('input[type="radio"]').is(":checked")
    ) {
